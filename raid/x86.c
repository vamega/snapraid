 ...
 *
 * -------------------------------------------------------------------------
 * CASE 3: Multiplication AFTER XOR, pre-scaling the accumulator
 * -------------------------------------------------------------------------
 * Used in highly pipelined architectures (like AVX-512) to improve 
 * instruction-level parallelism. Placing the multiplication AFTER the XOR 
 * in the loop allows the CPU to load and XOR the next disk while the 
 * complex doubling logic is still finishing. To ensure the math aligns 
 * correctly, the initial disk (D_l) must be pre-multiplied before entering 
 * the loop.
 *
 * Simplified Code:
 *   Q = D_l;
 *   Q = Q * 2;           // Pre-scale the initial disk
 *   
 *   for (d = l - 1; d > 0; --d) {
 *       Q = Q ^ D_d;     // Add current disk FIRST
 *       Q = Q * 2;       // Scale accumulator AFTER
 *   }
 *   
 *   // Final disk (D0) simply XORed, keeping coefficient 1
 *   Q = Q ^ D_0;
 */
