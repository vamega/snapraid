		...
		for (i = state->disklist, j = scanlist; i != 0; i = i->next, j = j->next) {
			...
			if (scan->count_equal == 0
				...
				if (!done) {
					...
					log_fatal(ESOFT, "WARNING! All the files previously present in disk '%s' at dir '%s'", disk->name, disk->mount_point);
				} else {
					log_fatal(ESOFT, ", disk '%s' at dir '%s'", disk->name, disk->mount_point);
				}
