...
static int devdereference_zfs(uint64_t device, const char* dir, tommy_list* devlist)
{
	...
	snprintf(cmd, sizeof(cmd), "zpool status -P %s 2>/dev/null", pool);
...
int fssnapshot_exists_zfs(const struct fssnapshot_struct* fss, const char* name)
{
...
#if HAVE_LINUX_DEVICE
int fssnapshot_exists_fs(struct fssnapshot_struct* fss, const char* name)
{
...
int fssnapshot_exists(struct fssnapshot_struct* fss, const char* name)
{
	...
	if (fss->magic == ZFS_SUPER_MAGIC) {
		return fssnapshot_exists_zfs(fss, name);
	} else {
		return fssnapshot_exists_fs(fss, name);
	}
#else
	(void)fss;
	(void)name;
