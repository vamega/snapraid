...
Getting Started
  ...
  Pooling
	...
	mergerfs tool, which is now the recommended option for Linux users in
...
Options
	...
	-w, --bw-limit RATE
		...
		such as K, M, G, or T (e.g., --bw-limit 1G).

	-t, --tail SIZE
		Limit file listing to those using no more than the specified
		tail size of the parity disks.
		You can use multipliers such as K, M, G, or T (e.g. --tail 1G).
		This option is only valid when used together with the `locate`
		command.
	...
	-W, --force-realloc-tail SIZE
		Works like -R, --force-realloc, but limited to the specified
		...
		the entire tail section (no blocks remain using it), the
	...
	--smartctl-path FILE
		Sets the absolute path to the `smartctl` binary.
		If not specified, SnapRAID searches the usual system paths.
		This option is only available on Unix-like systems.

	--zfs-path FILE
		Sets the absolute path to the `zfs` binary.
		If not specified, SnapRAID searches the usual system paths.
		This option is only available on Unix-like systems.

	--zpool-path FILE
		Sets the absolute path to the `zpool` binary.
		If not specified, SnapRAID searches the usual system paths.
		This option is only available on Unix-like systems.

	--bcachefs-path FILE
		Sets the absolute path to the `bcachefs` binary.
		If not specified, SnapRAID searches the usual system paths.
		This option is only available on Unix-like systems.
...
Configuration
  ...
  snapshot
	...
	See the SNAPSHOTS section for a detailed explanation of the snapshot
	lifecycle.
...
Snapshots Lifecycle
	...
	Subsequent `scrub`, `check`, and `fix` commands will use this
	pending snapshot as it matches the filesystem state recorded in
	...
	This historical information is only cleared once a sync finishes
	successfully and the final version of the .content file is saved.
